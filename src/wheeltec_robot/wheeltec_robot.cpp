...
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0)
{
  ...
  //23/12/18. 17:55.
  //changed:
  //"PowerVoltage" -> VOLTAGE_TOPIC
  //"odom" -> ODOM_TOPIC
  //"imu" -> IMU_TOPIC
  voltage_publisher = n.advertise<std_msgs::Float32>(VOLTAGE_TOPIC, 10); //Create a battery-voltage topic publisher //创建电池电压话题发布者
  odom_publisher    = n.advertise<nav_msgs::Odometry>(ODOM_TOPIC, 50); //Create the odometer topic publisher //创建里程计话题发布者
  imu_publisher     = n.advertise<sensor_msgs::Imu>(IMU_TOPIC, 20); //Create an IMU topic publisher //创建IMU话题发布者
  ...
  Cmd_Vel_Sub     = n.subscribe(VELOSITY_CALL_BACK_TOPIC,     100, &turn_on_robot::Cmd_Vel_Callback, this); 
